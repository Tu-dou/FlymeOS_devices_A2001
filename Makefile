...
vendor_saved_apps := Bluetooth KeyChain HTMLViewer UserDictionaryProvider BackupRestoreConfirmation \
		     ...
		     CaptivePortalLogin ConfigPanel OnePlusCamera com.qualcomm.location \
		     SettingsProvider CMSettingsProvider zhiyoo tudou BluetoothExt \
...
#-----------------------------------------------------------------------------
board_saved_files := lib/libwebviewchromium.so lib64/libwebviewchromium.so bin/bootanimation media/bootanimation.zip
...
override_property += \
    ...
    ro.product.model_romer=A2001_tudou \
